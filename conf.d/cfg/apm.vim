...
let g:apm_enable_ft = get(g:, 'apm_enable_ft', {'*':1})    " enable filetypes
let g:apm_enable_tab = get(g:, 'apm_enable_tab', 1)   " remap tab
let g:apm_omni = get(g:, 'apm_omni', 1)   " remap tab
...
function! s:apm_enable()
	...
	let b:apm_omni = get(g:, 'apm_omni', 1)
	if g:apm_enable_tab
		inoremap <silent><buffer><expr> <tab>
					\ pumvisible()? empty(get(v:, 'completed_item', {})) : "\<C-n>" :
					\ <SID>check_back_space() ? "\<tab>" :
					\ get(b:, 'apm_trigger', g:apm_trigger)
		inoremap <silent><buffer><expr> <tab>
					\ !pumvisible()? "\<Tab>":
					\ empty(get(v:, 'completed_item', {})) ? "\<C-n>" :
					\ "\<C-y>"
