...
function! s:link() abort
    for dir in s:editor_dirs
        if WINDOWS() && isdirectory(dir)
            ...
            " mklink
            let mklink_cmd = printf('mklink %s %s', dir . '\keybindings.json', $RTP_DIR . '\keybindings.json')
        ...
        elseif isdirectory(dir)
            ...
            call s:execute(rm_cmd)
            " ln -sf
            let ln_cmd = printf('ln -sf %s %s', $RTP_DIR . '/keybindings.json', dir . '/keybindings.json')
