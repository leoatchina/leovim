...
# Alt+H/J/K/L chord to that pane so Vim can run its <M-H/J/K/L> mappings.
# At a split edge, Vim calls back into herdr to cross the pane boundary.
# The same forwarding can be turned on for other TUIs via
# HERDR_NAV_PASSTHROUGH_RE. For any other foreground process, move herdr's
# pane focus directly.
#
