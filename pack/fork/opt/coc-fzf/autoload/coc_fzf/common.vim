...
function s:with_preview(placeholder, custom_cmd, wrap) abort
    ...
    if len(g:coc_fzf_preview) == 1
        let preview_window_opt = g:coc_fzf_preview[0]
        let toggle_key = '?'
    elseif len(g:coc_fzf_preview) > 1
        let preview_window_opt = g:coc_fzf_preview[0]
        let toggle_key = g:coc_fzf_preview[1]
    else
        return {}
    endif
    ...
    let wrapped_opts = fzf#vim#with_preview(placeholder_opt, preview_window_opt, toggle_key)
...
function coc_fzf#common#fzf_run_with_preview(opts, ...) abort
    ...
    let extra_opts  = has_key(extra, 'options') ? remove(extra, 'options') : ''
    let merged = extend(copy(a:opts), extra)
    call coc_fzf#common_fzf_vim#merge_opts(merged, extra_opts)
    call fzf#run(fzf#wrap(merged, 0))
endfunction
